...
from baseplate.crypto import legacy


class SigningTests(unittest.TestCase):

    def setUp(self):
        super(SigningTests, self).setUp()
        self.secret = VersionedSecret.from_simple_secret(
    ...
    def test_get_token(self):
        ...
        self.assertEqual(
            legacy.get_secret_token(self.secret, "test", 1, 1),
            "008c42a8952d949b9c95109eea5016bb00a5a0ac141b35a0691fe6a01f084241",
        )
        self.assertEqual(
            legacy.get_secret_token(self.secret, "test", 2, 1),
            "5081cd2623e0391da6b81d9590e9272e00bd17c29b4e3fb9b0044ff999cf5ae2",
        )
        self.assertRaises(
            AssertionError,
            lambda: legacy.get_secret_token(self.secret, "test", 1, 2),
        )
        self.assertEqual(
            legacy.get_secret_token(self.secret, "test2", 1, 1),
    ...
    def _assert_validity(self, body, header, success, error, **expected):
        request = mock.MagicMock(body=body, headers={})
        if header:
            request.headers[legacy.SIGNATURE_BODY_HEADER] = header
        signature = legacy.valid_post_signature(self.secret, request)
        self.assertEqual(signature.is_valid(), bool(success))
        if error:
            self.assertIn(error.name, [code for code, _ in signature.errors])
        else:
    ...
    def test_no_header(self):
        body = '{"user": "reddit", "password": "hunter2"}'
        self.assert_invalid(body, "", legacy.SigningError.INVALID_FORMAT)
    ...
    def test_garbage_header(self):
        ...
        self.assert_invalid(
            ...
            error=legacy.SigningError.INVALID_FORMAT,
        )
    ...
    def test_future_header(self):
        ...
        self.assert_invalid(
            ...
            error=legacy.SigningError.UNKOWN_GLOBAL_VERSION,
            global_version=2,
        )

    @mock.patch.object(legacy, "is_invalid_token", return_value=True)
    def test_invalid(self, _):
        ...
        self.assert_invalid(
            body,
            header=header,
            error=legacy.SigningError.INVALIDATED_TOKEN,
            global_version=1,
            version=version,
            platform=platform,
            has_mac=True,
    ...
    def test_invalid_header(self):
        ...
        self.assert_invalid(
            body,
            header=header,
            error=legacy.SigningError.UNPARSEABLE,
            global_version=1,
        )
    ...
    def test_expired_header(self):
        ...
        self.assert_invalid(
            body,
            header=header,
            error=legacy.SigningError.EXPIRED_TOKEN,
            global_version=1,
            platform=platform,
            version=version,
            has_mac=True,
